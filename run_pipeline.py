...
def main() -> None:
    logger.info("Starting full Olist data cleaning pipeline")
    ...
    try:
        ...
        contract_status = "✅ All passed" if fails == 0 else f"⚠️ {fails} failures"
    except Exception:
    ...
    logger.info("Summary:")
    logger.info(f"   Steps completed : {len(STEPS)}")
    logger.info(f"   Files processed : {len(PROCESSED_FILES)}")
    logger.info(f"   Schema contract : {contract_status}")
    logger.info(f"   Dtype audit     : {dtype_status}")
    logger.info("   Full details in: reports/clean_*.csv and docs/data_dictionary.md")
    logger.info("═" * 80)
